...
const actualizarImg = async (req, res= response) => {
    ...
    switch (coleccion) {
        case 'usuarios':
            ...
            if( !modelo ) {
                return res.status(400).json({msg:'No existe usuario con ID proporcionado'})
            }
            break;
        case 'productos':
            modelo = await Producto.findById(id);
            if( !modelo ) {
                return res.status(400).json({msg:'No existe producto con ID proporcionado'})
            }
            break;
...
const mostrarImagen = async  ( req, res= response ) => {
    ...
    pathPlaceHolder = path.join(__dirname, '../assets', 'no-image.jpg');

    res.sendFile(pathPlaceHolder);
    
}

const actImgCloud = async (req, res= response) => {
